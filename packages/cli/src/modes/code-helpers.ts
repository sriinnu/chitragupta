/**
 * @chitragupta/cli — Code mode helpers.
 *
 * Extracted from code.ts to stay within 450 LOC limit.
 * Contains types, formatting functions, stats computation,
 * and rendering functions for the code mode output.
 *
 ...
 */
export function computeStats(messages: readonly {
	role: string;
...
export function renderHeader(
	task: string, mode: string, providerId: string, modelId: string | undefined, useColor: boolean,
): string {
	const lines: string[] = [];
...
export function renderProgress(
	phases: { phase: string; message: string; elapsedMs: number }[], useColor: boolean,
): string {
	const lines: string[] = [];
	...
	for (const p of phases) {
		const mark = p.phase === "error" ? (useColor ? red("X") : "X") : (useColor ? green("V") : "V");
...
export function renderFiles(
	filesModified: string[], filesCreated: string[], useColor: boolean,
): string {
...
export function renderGit(
	git: { featureBranch: string | null; commits: string[] }, useColor: boolean,
): string {
...
export function renderTokenUsage(stats: UsageStats, useColor: boolean): string {
	const lines: string[] = [];
...
export function renderToolUsage(stats: UsageStats, useColor: boolean): string {
	const lines: string[] = [];
...
/** Render the full result summary for the code mode output. */
export function renderResult(
	...
	const status = result.success
		? (useColor ? bold(green("V Success")) : "V Success")
		: (useColor ? bold(red("X Failed")) : "X Failed");
	parts.push(`  Status: ${status} | Complexity: ${result.plan?.complexity ?? "unknown"}`);
	parts.push("");

	if (result.progressLog.length > 0) {
		const phaseMap = new Map<string, { phase: string; message: string; elapsedMs: number }>();
		for (const p of result.progressLog) phaseMap.set(p.phase, p);
	...
	if (result.plan && result.plan.steps.length > 0) {
		...
		for (const step of result.plan.steps) {
			const mark = step.completed ? (useColor ? green("V") : "V") : (useColor ? gray("o") : "o");
	...
	if (result.validationPassed !== undefined) {
		...
		parts.push(`  Result: ${result.validationPassed ? (useColor ? green("V passed") : "V passed") : (useColor ? red("X failed") : "X failed")}`);
		parts.push("");
	}
	...
	if (stats.totalCost > 0 || stats.totalToolCalls > 0) parts.push(renderTokenUsage(stats, useColor));
	if (stats.totalToolCalls > 0) parts.push(renderToolUsage(stats, useColor));
	...
	parts.push(useColor ? bold(dim(`Time: ${timeStr}`)) : `Time: ${timeStr}`);
