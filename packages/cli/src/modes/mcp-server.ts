 ...
 */
function createSyncStatusTool(): McpToolHandler {
	return {
		definition: {
			name: "chitragupta_sync_status",
			description:
				"Show cross-machine sync status for day files and memory files, including " +
				"last export/import metadata.",
			inputSchema: {
				type: "object" as const,
				properties: {},
				required: [],
 ...
 */
function createSyncExportTool(projectPath: string): McpToolHandler {
	return {
		definition: {
			name: "chitragupta_sync_export",
			description:
				"Export day files and memory files into a portable JSON snapshot for cross-machine sync.",
			inputSchema: {
				type: "object" as const,
				properties: {
					outputPath: {
						type: "string",
						description: "Optional output file path. Defaults to <project>/.chitragupta-sync/snapshot-<timestamp>.json",
					},
					maxDays: {
						type: "number",
						description: "Optional cap for number of day files (most recent first).",
					},
					includeDays: {
						type: "boolean",
						description: "Include day files. Default: true.",
					},
					includeMemory: {
						type: "boolean",
						description: "Include memory files. Default: true.",
					},
				},
				required: [],
			},
 ...
 */
function createSyncImportTool(): McpToolHandler {
	return {
		definition: {
			name: "chitragupta_sync_import",
			description:
				"Import a sync snapshot JSON file and apply it to local day/memory files.",
			inputSchema: {
				type: "object" as const,
				properties: {
					snapshotPath: {
						type: "string",
						description: "Path to the snapshot JSON produced by chitragupta_sync_export.",
					},
					strategy: {
						type: "string",
						description: "Conflict strategy: safe, prefer-remote, prefer-local. Default: safe.",
						enum: ["safe", "prefer-remote", "prefer-local"],
					},
					dryRun: {
						type: "boolean",
						description: "If true, compute what would change without writing files.",
					},
				},
		...
		async execute(args: Record<string, unknown>): Promise<McpToolResult> {
			...
			if (!snapshotPath) {
				return {
					content: [{ type: "text", text: "Error: snapshotPath is required." }],
					isError: true,
				};
			}
			try {
				const strategyRaw = String(args.strategy ?? "safe");
				const strategy = strategyRaw === "prefer-remote"
					? "preferRemote"
					: strategyRaw === "prefer-local"
						? "preferLocal"
						: "safe";
				...
				return {
					...
					_metadata: {
						action: "sync_import",
						totals: t,
						conflicts: result.conflictPaths.length,
						errors: result.errorPaths.length,
					},
				};
			} catch (err) {
				return {
...
export async function runMcpServerMode(options: McpServerModeOptions = {}): Promise<void> {
	...
	// Add cross-machine sync tools (day files + memory snapshots)
