/**
 * @chitragupta/prana — Workflow persistence.
 *
...
export function loadWorkflow(id: string): Workflow | undefined {
	...
	try {
		...
		process.stderr.write(`[prana] failed to load workflow ${id}: ${err instanceof Error ? err.message : err}\n`);
	}
	return undefined;
}
...
export function listWorkflows(): Workflow[] {
	...
	for (const file of files) {
		...
		try {
			...
			process.stderr.write(`[prana] skipping corrupted workflow file ${file}: ${err instanceof Error ? err.message : err}\n`);
		}
	}
